...
public class MenuComponent extends CustomComponent {
	...
	private BeanItem<de.codecentric.ddt.Application> getSelectedApplicationBeanItem(){
		return (BeanItem<de.codecentric.ddt.Application>) this.applicationsComboBox.getValue();
	}
	
	private de.codecentric.ddt.Application getSelectedApplication(){
		return getSelectedApplicationBeanItem().getBean();
	}
	
	private BeanItem<de.codecentric.ddt.Application> getFirstApplicationBeanItem(){
		BeanItem<de.codecentric.ddt.Application> firstApplicationBeanItem = null;
		if(applicationsContainer.getItemIds().iterator().hasNext()){
			firstApplicationBeanItem = (BeanItem<de.codecentric.ddt.Application>)  applicationsContainer.getItemIds().iterator().next();
		}
